...
(function ($, Drupal, _db, window) {
  ...
  var _selector = '[data-intense-gallery], .intense-gallery';
  ...
  function doIntense(i, elm) {
    ...
    $elm.off('click.ib').on('click.ib', '[data-intense-trigger], .intense', triggerIntense);
  }
  ...
  Drupal.behaviors.intense = {
    attach: function (context) {
      ...
      $(_selector, context).once('intense-gallery').each(doIntense);
    },
    detach: function (context, settings, trigger) {
      if (trigger === 'unload') {
        if ($(context).find(_selector).removeOnce('intense-gallery').length) {
          $(context).find(_selector).off('click.ib');
        }
      }
    }
  };
